...
- (void) processStartOfPartWithHeader:(MultipartMessageHeader*) header {
   ...
   // else {
        ...
        //Delegate here to show file transfer complete
        if([self.DelegateFileTransfer respondsToSelector:@selector(FileTransferStarted:)])
        {
            ////NSLog(@"File transfer Started : FileNAme %@",filename);
            [self.DelegateFileTransfer FileTransferStarted:filename];
...
- (void) processEndOfPartWithHeader:(MultipartMessageHeader*) header
{
    ...
    if([self.DelegateFileTransfer respondsToSelector:@selector(FileTransferEnded:)])
    {
        if(TransferStarted)
        {
            //NSLog(@"File transfer Ended");
            MultipartMessageHeaderField* disposition = [header.fields objectForKey:@"Content-Disposition"];
            NSString* filename = [[disposition.params objectForKey:@"filename"] lastPathComponent];
            [self.DelegateFileTransfer FileTransferEnded:filename];
